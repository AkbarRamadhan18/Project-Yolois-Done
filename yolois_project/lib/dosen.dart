...
class _DosenListScreenState extends State<DosenListScreen> {
  ...
  Future<List<Map<String, dynamic>>> _fetchDosenList() async {
    final response = await http.get(Uri.parse(
        'http://yo.yolois.my.id/dosen.php')); // Ganti URL dengan URL API Anda

    // Log data mentah dari server
    print('Response status: ${response.statusCode}');
    print('Response body: ${response.body}');

    if (response.statusCode == 200) {
      try {
        // Pastikan response body adalah JSON yang valid
        List<dynamic> responseData = jsonDecode(response.body);
        return List<Map<String, dynamic>>.from(responseData);
      } catch (e) {
        print('Error parsing JSON: $e');
        throw Exception('Failed to parse dosen list');
      }
    } else {
                        ...
                        ListTile(
                          title: Row(
                            children: [
                              ...
                              SizedBox(
                                  width: 20), // Penambahan jarak horizontal
                              Expanded(
                                flex: 3,
                                child: Text(
